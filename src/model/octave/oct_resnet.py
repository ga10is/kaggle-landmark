...
from .OctaveConv1 import FirstOctaveCBR, LastOCtaveCB, OctaveCB, OctaveCBR
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
        ...
        self.first = First
        if self.first:
            self.ocb1 = FirstOctaveCBR(inplanes, width, kernel_size=(
                1, 1), norm_layer=norm_layer, padding=0)
        else:
            self.ocb1 = OctaveCBR(inplanes, width, kernel_size=(
                1, 1), norm_layer=norm_layer, padding=0)

        self.ocb2 = OctaveCBR(width, width, kernel_size=(
            3, 3), stride=stride, groups=groups, norm_layer=norm_layer)

        self.ocb3 = OctaveCB(width, planes * self.expansion,
...
class OCtaveResNet(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, stride=1, norm_layer=None, First=False):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                OctaveCB(in_channels=self.inplanes, out_channels=planes * block.expansion,
    ...
    def _make_last_layer(self, block, planes, blocks, stride=1, norm_layer=None):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                LastOCtaveCB(in_channels=self.inplanes, out_channels=planes * block.expansion,
                             kernel_size=(1, 1), stride=stride, padding=0)
            )
